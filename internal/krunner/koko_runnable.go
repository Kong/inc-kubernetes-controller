...
import (
	...
	"github.com/kong/inc-kubernetes-controller/internal/koko/info"
	"github.com/kong/inc-kubernetes-controller/internal/koko/metrics"
	metricsv2 "github.com/kong/inc-kubernetes-controller/internal/koko/metrics/v2"
	...
	"github.com/kong/inc-kubernetes-controller/internal/koko/server/health"
	...
	"github.com/kong/inc-kubernetes-controller/internal/koko/util"
	"go.uber.org/zap"
	"google.golang.org/grpc"
	"google.golang.org/grpc/credentials/insecure"
)

// Run is similar to Koko's standard cmd.run, but with several components initialized (but not started) beforehand
func (k *KokoRunnable) Run(ctx context.Context) error {
	...
	// TODO this will also have an internal interface accessible to the controllers. do we want to expose a writeable
	// HTTP API also? mixed configuration is permitted in KIC DB mode, but we also strongly suggest against it
	// Probably not, since there's no way to persist changes on leadership change when using in-memory sqlite
	...
	// TODO ideally the components started here would also be Runnables and managed by the controller-runtime manager
	// it's not immediately obvious how to make that fit with the existing Koko code, which will presumably continue
	// using gang. the interfaces are _similar_, so it's possible we could stick Start shims on each of them, but we'd
	// need to modify this to return them. further not sure how they don't start until here, but this invokes grpc.Dial
	// earlier
	errCh := g.Run(ctx)
